...
} from "../extensions/weasley-autoresearch/shortcuts.ts";
import autoresearchExtension from "../extensions/weasley-autoresearch/index.ts";

test("autoresearch shortcuts default to the documented bindings when config is absent", async () => {
  const agentDir = await mkdtemp(join(tmpdir(), "weasley-autoresearch-test-"));
  try {
    const configPath = autoresearchShortcutsConfigPath(agentDir);
    const shortcuts = resolveAutoresearchShortcuts(configPath);

    assert.equal(configPath, join(agentDir, "extensions", "weasley-autoresearch.json"));
...
test("malformed shortcut config warns and falls back to defaults", async () => {
  ...
  try {
    ...
    assert.match(warnings[0], /weasley-autoresearch.*config/i);
...
test("invalid known shortcut fields warn and fall back to defaults for the whole file", async () => {
  ...
  try {
    ...
    assert.equal(warnings.length, 1);
    assert.match(warnings[0], /invalid weasley-autoresearch config/i);
  } finally {
    console.warn = previousWarn;
    await rm(agentDir, { recursive: true, force: true });
  }
