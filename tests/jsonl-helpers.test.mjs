...
import { appendHookLogEntryIfConfigured } from "../extensions/weasley-autoresearch/hooks.ts";
import {
...
} from "../extensions/weasley-autoresearch/jsonl.ts";
...
test("hook observability does not create jsonl before config exists", () => {
  const tempDir = fs.mkdtempSync(path.join(tmpdir(), "weasley-autoresearch-hooks-"));
