...
An extension for **[pi](https://pi.dev/)** — an AI coding agent that runs in your terminal. weasley-autoresearch gives pi the tools and workflow to run autonomous optimization loops: try an idea, benchmark it, keep improvements, revert regressions, repeat.
...
<img width="1736" height="518" alt="weasley-autoresearch" src="https://github.com/user-attachments/assets/5078aa31-3530-406a-85fc-bdeff98679a6" />
...
`<agent-dir>/extensions/weasley-autoresearch.json`. `<agent-dir>` is the active pi profile
...
```bash
cp -r extensions/weasley-autoresearch ~/.pi/agent/extensions/
...
After 3+ experiments in a session, weasley-autoresearch computes a **confidence score** — how the best improvement compares to the session's noise floor. This helps distinguish real gains from benchmark jitter, especially on noisy signals like ML training, Lighthouse scores, or flaky benchmarks.
